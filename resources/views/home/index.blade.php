...
@section('info')
    ...
    <title>{{Config::get('web.web_site')}} - {{Config::get('web.web_site')}}</title>
...
@section('content')
        ...
        <div class="box">
            ...
            <ul>
                ...
                @foreach($pics as $p)
                    ...
                    <li><a href="{{url('a/'.$p->article_id)}}"  target="_blank"><img src="{{url($p->article_thumb)}}"></a><span>"{{$p->article_title}}"</span></li>
    ...
    <article>
        ...
        <div class="bloglist left">
            ...
            <ul>
                <p>{{$d->article_description}}</p>
                <a title="{{$d->article_title}}" href="{{url('a/'.$d->article_id)}}" target="_blank" class="readmore">阅读全文>></a>
            </ul>
            <p class="dateview"><span>{{date('Y-m-d',$d->article_time)}}</span><span>作者：{{$d->article_editor}}</span></p>
